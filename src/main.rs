    ...
    for line in md.lines() {
        ...
        if line.starts_with("| ") || (s.in_table && line.starts_with('|')) {
            ...
            for cell in line.trim_matches('|').split('|') {
                s.out
                    .push_str(&format!("<{0}>{1}</{0}>", tag, fmt_inline(cell.trim())));
            }
            s.out.push_str("</tr>\n");
            continue;
        }
...
async fn docs_handler(State(state): State<AppState>, Path(path): Path<String>) -> Response {
...
async fn main() {
    ...
    let nav = Arc::new(vec![
        ...
        NavItem {
            ...
            path: "phase-01-core-authentication",
            category: "Phases",
            icon: "&#x1F510;",
        },
        NavItem {
            title: "Phase 2: User Auth",
            path: "phase-02-user-authentication",
            category: "Phases",
            icon: "&#x1F464;",
        },
        NavItem {
            title: "Phase 3: 2FA",
            path: "phase-03-two-factor-auth",
            category: "Phases",
            icon: "&#x1F4F1;",
        },
        NavItem {
            title: "Phase 4: OAuth",
            path: "phase-04-oauth-integration",
            category: "Phases",
            icon: "&#x1F310;",
        },
        NavItem {
            title: "Phase 5: Email",
            path: "phase-05-email-verification",
            category: "Phases",
            icon: "&#x1F4E7;",
        },
        NavItem {
            title: "Phase 6: RBAC",
            path: "phase-06-rbac-advanced",
            category: "Phases",
            icon: "&#x1F465;",
        },
        NavItem {
            title: "Phase 7: API Polish",
            path: "phase-07-api-polish",
            category: "Phases",
            icon: "&#x2728;",
        },
        NavItem {
            title: "Phase 8: Security",
            path: "phase-08-rate-limiting-security",
            category: "Phases",
            icon: "&#x1F6E1;",
        },
        NavItem {
            title: "Phase 9: CLI Spec",
            path: "phase-09-cli-commands",
            category: "Phases",
            icon: "&#x1F4BB;",
        },
        NavItem {
